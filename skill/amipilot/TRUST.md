# AmiPilot Trust & Security Model

## What AmiPilot Does

AmiPilot is a local HTTP server that controls a Chromium browser via Playwright. It:
...
- Optional Bearer token authentication (`AMIPILOT_TOKEN`)
...
1. **Always set `AMIPILOT_TOKEN`** when the API is accessible to other processes
